...
function refreshPage(){
  ...
  let currCourse = courses[random];  
  let currCourseid = currCourse.course_id
  console.log("Current Course ID", currCourseid);
  let avgGPAitem = avgGPA(currCourseid)
  console.log("Current Course GPA", avgGPAitem); 
  let course_popup = document.querySelector(".course-rec");
...
function avgGPA(course_id) {
  ...
  .then((data) => { // i know you've done this with data but alex explicitly said it needs to be avail courses
    ...
    const gpa = TotalClassGPA.toFixed(2)
